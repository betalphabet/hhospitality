...
class DraggableMap {
    constructor() {
        ...
        this.getMapScale = () => {
            if (window.innerWidth > 991) {
                // 電腦版：根據視窗大小動態計算縮放比例，確保地圖完整顯示
                const mapWidth = 5600;
                const mapHeight = 4200;
                const windowWidth = window.innerWidth;
                const windowHeight = window.innerHeight;

                // 計算寬度和高度的縮放比例，取較小值以確保完整顯示 (contain)
        ...
        try {
            ...
            const response = await fetch(
                currentUrl.includes('[::]:8000')
                    ? 'http://[::]:8000/map/en_data.json'
                    : 'https://hhospitality.group/map/en_data.json'
            );
            if (!response.ok) {
    ...
    showTooltip(marker) {
        ...
        tooltip.querySelector('.tooltip-address').textContent = marker.address || 'Address not available';
    ...
    centerMap() {
        ...
        if (window.innerWidth > 991) {
            // 更新地圖圖片縮放比例
            this.mapImage.style.transform = `scale(${mapScale})`;

            this.currentX = (containerRect.width - mapDimensions.width) / 2;
            this.currentY = (containerRect.height - mapDimensions.height) / 2;
            this.updateMapPosition();
            return;
        } else {
            // 手機版：回復預設 CSS 樣式
    ...
    showPhotoPanel(locationName, photoFolder) {
        ...
        // 設置標題
        photoPanelTitle.textContent = `${locationName} - Photo Gallery`;
    ...
    async loadPhotos(photoFolder, container) {
        try {
                ...
                for (let i = 1; i <= maxPhotos; i++) {
                    for (const ext of photoExtensions) {
                        ...
                        img.onload = () => {
                            ...
                            photoItem.innerHTML = `<img src="${imagePath}" alt="${folderName} Photo ${i}" loading="lazy">`;
                ...
                if (folderName === 'jane') {
                    ...
                    specialImages.forEach((fileName, index) => {
                        ...
                        img.onload = () => {
                            const photoItem = document.createElement('div');
                            photoItem.className = 'photo-item';
                            photoItem.innerHTML = `<img src="${imagePath}" alt="${folderName} Photo ${index + 1}" loading="lazy">`;
                            photoItem.addEventListener('click', () => this.openFullscreenModal(imagePath));
                            container.appendChild(photoItem);
                        };
                        img.src = imagePath;
            ...
            container.innerHTML = '<p style="text-align: center; color: #666; padding: 20px;">Unable to load photos</p>';
        }
    }
